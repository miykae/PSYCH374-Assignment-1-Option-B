...
function showResults() {
    ...
    const s1 = result.details.scenario1 ?? 0;
    const s2 = result.details.scenario2 ?? 0;
    const signTracking = s1; // Phase1 cue + Phase2 reward
    const goalTracking = s2; // Phase2 reward only
    const scenarioText = (signTracking + goalTracking) > 0
        ? `<p><strong>Sign-tracking:</strong> ${signTracking} | <strong>Goal-tracking:</strong> ${goalTracking}</p>`
        : '';
