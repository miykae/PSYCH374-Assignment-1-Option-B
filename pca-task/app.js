...
async function runTask() {
    ...
    for (let i = 0; i < config.totalTrials; i++) {
        ...
        // Phase 1: Cue only — star appears, cue stays for full duration (animal model)
        itiMsg.textContent = 'Go';
        ...
        goalStim.title = trial.isDevalued ? 'Box' : 'Gift box';
        ...
        const phase1Result = await waitForPhase1Cue(signStim, config.cueDuration);
        const phase1SignClick = phase1Result.clicked;

        // Phase 2: Cue disappears, reward appears
        signStim.classList.add('hidden');
        goalStim.classList.remove('hidden');
        const phase2Result = await waitForPhase2Goal(goalStim);
        // Phase 2 has no timeout — we only proceed when user clicks the reward

        // Scenarios: 1 = cue + reward (sign-tracking), 2 = reward only (goal-tracking)
        const scenario = phase1SignClick ? 1 : 2;
        const responseType = phase1SignClick ? 'sign' : 'goal';
        const latency = phase2Result.timestamp - PCA_TASK.state.cueOnset; // time from cue onset to reward click

        PCA_TASK.recordResponse(responseType, latency, { scenario, phase1SignClick });
        score++;
        document.getElementById('score-display').textContent = `Points: ${score}`;
        // Behavioural safeguards: 3 consecutive Phase 1 star clicks → tip; 3 more (after tip) → devalue
        if (responseType === 'sign') {
            consecutiveSignClicks++;
            if (consecutiveSignClicks >= 3 && !tipShown) {
                const tipBanner = document.getElementById('tip-banner');
                tipBanner.textContent = 'Tip: Try going for just the gift box instead!';
        ...
        const postTrialDelay = responseType === 'goal' ? 250 : 600;
...
/** Phase 1: Cue only. Runs full duration, records if star was clicked. Cue stays visible entire time. */
function waitForPhase1Cue(signStim, durationMs) {
    return new Promise(resolve => {
        ...
        signStim.addEventListener('click', handler);
        setTimeout(() => {
            signStim.removeEventListener('click', handler);
            resolve({ clicked, timestamp });
        }, durationMs);
    });
}

/** Phase 2: Reward only. Resolves only on goal click (no timeout). */
function waitForPhase2Goal(goalStim) {
    return new Promise(resolve => {
        const handler = (e) => {
            e.preventDefault();
            e.stopPropagation();
            goalStim.removeEventListener('click', handler);
            resolve({ clicked: true, timestamp: performance.now() });
        };
        goalStim.addEventListener('click', handler);
    });
}
...
function showResults() {
    ...
    let devalText;
    if (deval) {
        const avgNorm = Math.round(deval.avgLatencyNormal);
        const avgDev = Math.round(deval.avgLatencyDevalued);
        const diff = Math.round(deval.difference);
        let diffNote = '';
        if (diff > 0) diffNote = ` You were ${diff} ms slower on devalued trials.`;
        else if (diff < 0) diffNote = ` You were ${Math.abs(diff)} ms faster on devalued trials.`;
        devalText = `<p><strong>Devaluation sensitivity:</strong> Average latency to reward — ${avgNorm} ms on normal trials (gift box) vs ${avgDev} ms on devalued trials (plain box).${diffNote}</p>`;
    } else {
        devalText = '<p><strong>Devaluation sensitivity:</strong> Not enough data — need responses on both devalued trials (plain box) and normal trials (gift box) to compute.</p>';
    }

    const signTracking = result.details.scenario1 ?? 0;
    const goalTracking = result.details.scenario2 ?? 0;
    ...
    summary.innerHTML = `
        ...
        <p>Sign-tracking: ${signTracking} | Goal-tracking: ${goalTracking}</p>
        ${result.details.avgLatencySign != null ? `<p>Avg latency to reward (sign trials): ${result.details.avgLatencySign} ms</p>` : ''}
        ${result.details.avgLatencyGoal != null ? `<p>Avg latency to reward (goal trials): ${result.details.avgLatencyGoal} ms</p>` : ''}
        ${devalText}
    `;
    ...
    return `
        <div class="viz-container" style="margin: 1rem 0;">
            ...
            <div id="viz-trial-plot" class="viz-expandable" style="display:none">
                ...
                <p class="viz-caption">Each bar = one trial. Orange = sign-tracking (clicked cue then reward), Green = goal-tracking (reward only)</p>
            ...
            <div id="viz-latency-plot" class="viz-expandable" style="display:none">
                ...
                <p class="viz-caption">Time from cue onset to reward click for each trial</p>
