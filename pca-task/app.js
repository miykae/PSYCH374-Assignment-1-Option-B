...
async function runTask() {
    ...
    for (let i = 0; i < config.totalTrials; i++) {
        ...
        itiMsg.textContent = 'Click the star or the treasure chest.';
        ...
        // Show treasure chest shortly after star (cue predicts reward)
        goalStim.classList.add('hidden');
        await sleep(500);
        ...
        if (response) {
            ...
            if (response.type === 'sign') {
                ...
                if (consecutiveSignClicks >= 3 && !tipShown) {
                    ...
                    tipBanner.textContent = 'Tip: Try going for the treasure chest instead!';
...
function showResults() {
    ...
    const devalText = deval ? (deval.pSignDevalued === 0 && deval.pSignNormal === 0
        ? '<p>Devaluation sensitivity: You clicked the goal on 100% of both devalued and normal trials — consistent goal-tracking.</p>'
        : `<p>Devaluation sensitivity: Proportion of sign (vs goal) responses — ${(deval.pSignDevalued * 100).toFixed(0)}% on devalued trials vs ${(deval.pSignNormal * 100).toFixed(0)}% on normal trials.</p>`) : '';
    ...
    return `
        <div class="viz-container" style="margin: 1rem 0;">
            ...
            <div id="viz-trial-plot" class="viz-expandable" style="display:none">
                ...
                <p class="viz-caption">Each bar = one trial. Orange = star (sign), Green = treasure chest (goal), Gray = no response</p>
